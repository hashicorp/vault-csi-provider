...
* Vault CSI Provider now creates a Kubernetes secret with an HMAC key to produce consistent hashes for secret versions.
  ...
  * Supports creating the secret with custom metadata via:
    * `-hmac-secret-name`
    * `-hmac-secret-labels-file`
    * `-hmac-secret-annotations-file`.

    The file arguments should refer to a file with the same format as created by a Downward API volume populated with `metadata.labels`
    or `metadata.annotations`: https://kubernetes.io/docs/concepts/workloads/pods/downward-api/#downwardapi-fieldRef.
